...
namespace Core
{
    public class Board : MonoBehaviour
    {
        ...
        [Header("Audio")] public AudioClip moveClip;
        public AudioClip clearClip;
        private AudioSource audioSource;

        [Header("Settings")] public Vector2Int boardSize = new Vector2Int(10, 20);
        public Vector3Int spawnPosition = new Vector3Int(0, 8, 0);

        [Header("Visuals")] public Color gridColor = new Color(0.1f, 0.1f, 0.1f, 1f); // Dark grey

        [Header("Game State")] public int score;
        ...
        private void Start()
        {
            ...
            if (backgroundTilemap == null)
                backgroundTilemap = transform.Find("Grid/BackgroundTilemap")?.GetComponent<Tilemap>();
        ...
        private void UpdateHUD()
        {
            if (cachedHUD != null)
            {
                cachedHUD.UpdateScore(score);
                cachedHUD.UpdateNext(nextBlocks.ToList());
                cachedHUD.UpdateHold(heldBlock);
            }
        }
        ...
        public void SpawnBlock(BlockData data)
        {
            ...
            // Only check if there's a collision with existing tiles, not bounds
            // This allows pieces to spawn partially above the visible board
            if (!IsValidSpawnPosition(activeBlock, Vector3Int.RoundToInt(blockObj.transform.position)))
            {
                Destroy(blockObj);
                GameOver();
            }
        ...
        // Special validation for spawn - allows blocks above the board but checks for collisions
        private bool IsValidSpawnPosition(Block block, Vector3Int position)
        {
            RectInt bounds = new RectInt(new Vector2Int(-boardSize.x / 2, -boardSize.y / 2), boardSize);
            ...
            foreach (Vector2Int cell in block.data.cells)
            {
                ...
                Vector3Int tilePos = Vector3Int.RoundToInt(position + cellOffset);

                // Only check horizontal bounds and tile collisions
                // Allow vertical spawning above the board
                if (tilePos.x < bounds.xMin || tilePos.x >= bounds.xMax) return false;
